...
pub fn vtable_for<'a>(node: &'a JSRef<'a, Node>) -> &'a VirtualMethods + 'a {
    match node.type_id() {
        ElementNodeTypeId(HTMLAnchorElementTypeId) => {
            let element: &'a JSRef<'a, HTMLAnchorElement> = HTMLAnchorElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLAreaElementTypeId) => {
            let element: &'a JSRef<'a, HTMLAreaElement> = HTMLAreaElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLBodyElementTypeId) => {
            let element: &'a JSRef<'a, HTMLBodyElement> = HTMLBodyElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLButtonElementTypeId) => {
            let element: &'a JSRef<'a, HTMLButtonElement> = HTMLButtonElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLCanvasElementTypeId) => {
            let element: &'a JSRef<'a, HTMLCanvasElement> = HTMLCanvasElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLFieldSetElementTypeId) => {
            let element: &'a JSRef<'a, HTMLFieldSetElement> = HTMLFieldSetElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLImageElementTypeId) => {
            let element: &'a JSRef<'a, HTMLImageElement> = HTMLImageElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLIFrameElementTypeId) => {
            let element: &'a JSRef<'a, HTMLIFrameElement> = HTMLIFrameElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLInputElementTypeId) => {
            let element: &'a JSRef<'a, HTMLInputElement> = HTMLInputElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLLinkElementTypeId) => {
            let element: &'a JSRef<'a, HTMLLinkElement> = HTMLLinkElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLObjectElementTypeId) => {
            let element: &'a JSRef<'a, HTMLObjectElement> = HTMLObjectElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLOptGroupElementTypeId) => {
            let element: &'a JSRef<'a, HTMLOptGroupElement> = HTMLOptGroupElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLOptionElementTypeId) => {
            let element: &'a JSRef<'a, HTMLOptionElement> = HTMLOptionElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLSelectElementTypeId) => {
            let element: &'a JSRef<'a, HTMLSelectElement> = HTMLSelectElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLStyleElementTypeId) => {
            let element: &'a JSRef<'a, HTMLStyleElement> = HTMLStyleElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(HTMLTextAreaElementTypeId) => {
            let element: &'a JSRef<'a, HTMLTextAreaElement> = HTMLTextAreaElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(ElementTypeId) => {
            let element: &'a JSRef<'a, Element> = ElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        ElementNodeTypeId(_) => {
            let element: &'a JSRef<'a, HTMLElement> = HTMLElementCast::to_ref(node).unwrap();
            element as &'a VirtualMethods + 'a
        }
        _ => {
